...
func (s *compServices) CachePredict(image []byte, data dto.MLResponse) error {
	...
	err = rdb.Set(ctx, string(image), jsonValue, time.Hour * 2).Err()
	if err != nil {
...
func (s *compServices) GetPredictCache(image []byte) (*dto.MLResponse, error) {
	rdb := config.ConnectRedis()
	ctx := context.Background()

	result, err := rdb.Get(ctx, string(image)).Result()
	if err != nil {
		if err == redis.Nil {
			return nil, nil
		}
